...
CHARACTER(8)  ::  $giorno(2),$ieri,$oggi,$domani
CHARACTER(7)  ::  $latrif,$lonrif,$latrif2,$lonrif2
CHARACTER     ::  dum*2,$kd*1,$ng*1,$rvFWIout*11,$ipt*4,$riga*50,$ur*5,$met*5
...
IF (iRetCode/=0) THEN
  ...
  STOP 1
ENDIF
READ(18,"(a8,2x,a8,2x,a8)")  $ieri,$oggi,$domani        ! data di ieri, oggi (ng=1) e data di domani (ng=2)
CLOSE(18)
...
OPEN(14,file=TRIM(sPATH0)//"log/fwigrid_for_"//$oggi//".log",status="unknown")
...
READ($ieri(7:8),"(i2)") iDD0
READ($ieri(5:6),"(i2)") iMMM0
READ($ieri(1:4),"(i4)") iYYYY0
READ($oggi(7:8),"(i2)") iDD1
READ($oggi(5:6),"(i2)") iMMM1
READ($oggi(1:4),"(i4)") iYYYY1
READ($domani(7:8),"(i2)") iDD2
READ($domani(5:6),"(i2)") iMMM2
READ($domani(1:4),"(i4)") iYYYY2
...
! Esempio: ffmc_lami_$data.txt = file x,y,z con i valori di ffmc calcolati da misure interpolate
!                                per il giorno $data, (rUNDEF=0 fuori regione, nei punti con neve, nei punti non bruciabili, in caso di dati invalidi:
!                                il rUNDEF=0 deriva dall'operazione di trasformazione coordinate con GRASS)
!                           N.B. viene ricavato dai corispondenti file ffmc_$data.txt convertendoli nel grigliato
!                                COSMO I7 in coordinate lat lon.

DO kk=1,3
  OPEN(unit=iFILIN(kk),file=TRIM(ADJUSTL(sPATH1))//TRIM(sFILO(kk))//$ieri//'.txt',status='old',IOSTAT=iRetCode)
  IF (iRetCode/=0) THEN
...
!
! Esempio: t_lami_$data_ng.txt = file x,y,z con i valori meteo previsti dal CosmoI7 per l'emissione del giorno $data (=$oggi),
!                                e per il giorno ng di previsione (ng=1 -> $oggi; ng=2-> $domani)

DO km=1,4
  DO kd=1,2
    WRITE($kd,"(i1)") kd
    sFILIM(km,kd)=TRIM(ADJUSTL(sFILIM(km,kd)))//$oggi//'_'//$kd//'.txt'
  END DO
END DO
...
!  scrittura file input meteo per controllare 1 volta che la lettura sia corretta!!!! poi commentare
    ...
    DO km=1,4
      sFILICM(km,1)=TRIM(ADJUSTL(sFILICM(km,1)))//$oggi//'_1_ctrl.txt'
      sFILICM(km,2)=TRIM(ADJUSTL(sFILICM(km,2)))//$oggi//'_2_ctrl.txt'
    END DO
    ...
    DO km=1,4
      DO kd=1,2
        ...
        DO ipt=1,iNP
          WRITE($ipt,"(i4)") ipt
          WRITE($latrif,"(f7.3)") latrif(ipt)
          WRITE($lonrif,"(f7.3)") lonrif(ipt)
          WRITE($latrif2,"(f7.3)") latrif2(ipt)
          WRITE($lonrif2,"(f7.3)") lonrif2(ipt)
          WRITE($met,"(f5.1)") met(ipt,km,kd)
          $riga=TRIM(ADJUSTL($ipt))//','//TRIM(ADJUSTL($latrif))//','//TRIM(ADJUSTL($lonrif))//','//TRIM(ADJUSTL($latrif2))//','&
            //TRIM(ADJUSTL($lonrif2))//','//TRIM(ADJUSTL($met))

          WRITE(iFILICM(km,kd),*) TRIM(ADJUSTL($riga))
        END DO
        CLOSE(iFILICM(km,kd))
      END DO
    END DO
...
! aggiungo 2 file per umidita' relativa
    ...
    OPEN(unit=49,file=TRIM(ADJUSTL(sPATH2))//'ctrl/ur_lami_'//$oggi//'_1_ctrl.txt',status='unknown',IOSTAT=iRetCode)
    IF (iRetCode/=0) THEN
      PRINT *,'fwigrid_for: error opening file <','ur_lami_'//$oggi,'>'
      STOP 1
    ENDIF
    ...
    DO ipt=1,iNP
      ...
      WRITE($ur,"(f5.1)") ur
      $riga=TRIM(ADJUSTL($ipt))//','//TRIM(ADJUSTL($latrif))//','//TRIM(ADJUSTL($lonrif))//','//TRIM(ADJUSTL($latrif2))//','&
            //TRIM(ADJUSTL($lonrif2))//','//TRIM(ADJUSTL($ur))

      WRITE(49,*) TRIM(ADJUSTL($riga))
    END DO
    CLOSE(49)

    OPEN(unit=50,file=TRIM(ADJUSTL(sPATH2))//'ctrl/ur_lami_'//$oggi//'_2_ctrl.txt',status='unknown',IOSTAT=iRetCode)
    IF (iRetCode/=0) THEN
      PRINT *,'fwigrid_for: error opening file <','ur_lami_'//$domani,'>'
      STOP 1
    ENDIF
    WRITE(50,*)
    DO ipt=1,iNP
...
!!!!!!! *****  scrivere file input indici per controllare 1 volta che la lettura sia corretta!!!! poi commentare
   ...
   DO kk=1,3
     OPEN(unit=iFILICN(kk),file=TRIM(ADJUSTL(sPATH1))//'ctrl/'//TRIM(sFILO(kk))//$ieri//'_ctrl.txt',status='unknown',IOSTAT=iRetCode)
     IF (iRetCode/=0) THEN
       PRINT *,'fwigrid_for: error opening file <',TRIM(sFILO(kk))//$ieri//'_ctrl','>'
       STOP 1
     ENDIF
     WRITE(iFILICN(kk),*) 
     DO ipt=1,iNP
...
! verifica presenza dati meteo non validi
    ...
    IF(t.eq.rUNDEF.or.h.eq.rUNDEF.or.w.eq.rUNDEF.or.r.eq.rUNDEF) THEN
      ...
      IF(vrif(ipt).ne.rUNDEF_GRAS) THEN
        ...
        WRITE(14,*)  "Oggi: ",$giorno(ng), " Punto di griglia: ",ipt
      END IF
      GOTO 40
    END IF
...
! *********   fine ciclo sui punti di griglia   *******
  ...
  IF(inorm.eq.0) THEN
    WRITE(14,*) 
    WRITE(14,*) $giorno(ng)," Giorno elaborato senza errori"
    WRITE(*,*) $giorno(ng)," Giorno elaborato senza errori"
  ELSE
    WRITE(14,*) 
    WRITE(14,*) $giorno(ng)," Giorno elaborato con errori"
    WRITE(*,*) $giorno(ng)," Giorno elaborato con errori"
    inorm=0
  END IF
...
!
! Esempio: ffmc_lami_$data_ng.txt = matrice con i valori di ffmc previsti calcolati per $data, intesa come giorno di previsione numero ng
!                                   (rUNDEF fuori regione e in caso di dati invalidi)
!                                  Tale file verra' trattato con GRASS per aggegazione areale e graficazione
!                                  N.B. tali valori sono calcolati usando, oltre ai dati meteo, il file ffmc_lami_($data-1).txt per ng=1 e
!                                       l'array rvFWIout(ng=1) per ng=2

  WRITE($ng,"(i1)") ng

  DO kk=1,6
    OPEN(unit=iFILO(kk,ng),file=TRIM(ADJUSTL(sPATH3))//TRIM(sFILO(kk))//$giorno(ng)//'_'//$ng//'.txt',status='unknown') 
    ...
    DO ipt=1,iNP
      ...
      WRITE($latrif2,"(f7.3)") latrif2(ipt)
      WRITE($lonrif2,"(f7.3)") lonrif2(ipt)
      WRITE($rvFWIout,"(f11.5)") rvFWIout(ipt,kk,ng)
      $riga=TRIM(ADJUSTL($ipt))//','//TRIM(ADJUSTL($latrif))//','//TRIM(ADJUSTL($lonrif))//','//TRIM(ADJUSTL($latrif2))//','&
            //TRIM(ADJUSTL($lonrif2))//','//TRIM(ADJUSTL($rvFWIout))

      WRITE(iFILO(kk,ng),*) TRIM(ADJUSTL($riga))
    END DO
    CLOSE(iFILO(kk,ng))
  END DO
...
!
! Esempio: ffmc_c_lami_$data_ng.txt = matrice con le classi di ffmc previste calcolate per $data, intesa come giorno di previsione numero ng
!                                    (rUNDEF fuori regione e in caso di dati invalidi)
!                             Tale file verra' trattato con GRASS per la produzione di mappe (?)
!                             N.B. tali valori sono calcolati usando il file ffmc_lami_($data-1).txt e il file fwiclassi.ini per ng=1 e l'array
!                                  rvFWIout per ng=2

  DO kk=1,6
    OPEN(unit=iFILOC(kk,ng),file=TRIM(ADJUSTL(sPATH3))//TRIM(sFILOC(kk))//$giorno(ng)//'_'//$ng//'.txt',status='unknown') 
