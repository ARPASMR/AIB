...
INTEGER       ::  ik,j,m,ind,imo,lmon(12),isciolgo(iMAX_Y_DIM,iMAX_X_DIM),inorm,idef
INTEGER       ::  iSNOWold(iMAX_Y_DIM,iMAX_X_DIM),iSNOWtod(iMAX_Y_DIM,iMAX_X_DIM)
INTEGER       ::  iFILO(6),iFILOC(6),iFILIT(3),iFILI(3),iFILOT(3),coord(2,NPUNTIMAX)
INTEGER       ::  iditot(iMAX_Y_DIM,iMAX_X_DIM),iUNDEF,isalta,ir,ic,ll,npunti
...
INTEGER       ::  iCOL,iRIG,iCOLok,iRIGok,id,ilomb(iMAX_Y_DIM,iMAX_X_DIM)
INTEGER       ::  isnowper(iMAX_Y_DIM,iMAX_X_DIM),isnowper_lenght,istdby
CHARACTER     ::  infmt*30,inpath*70,outpath*70
CHARACTER(2)  ::  ihr,ida,imo1,ihri,idai,imoi
CHARACTER     ::  dum*2,giorno*8,ieri*8,sTEMP*12,iyr*4,iyri*4
CHARACTER(70) ::  sPATH0,sPATH1,sPATH2
CHARACTER(30) ::  sFILO(6),sFILOC(6), sFILIT(3)
...
REAL(8)       ::  rvFWIana(iMAX_Y_DIM,iMAX_X_DIM,3)
REAL          ::  ws(iMAX_Y_DIM,iMAX_X_DIM)
REAL          ::  fmcl(12,5),dmcl(12,5),dcl(12,5),iscl(12,5),bucl(12,5),fwcl(12,5)
REAL          ::  mcl(12,5),dscl(12,5),idisoglia(4),idi(4),rUNDEF_SNOW
REAL          ::  ind_tmp(iMAX_Y_DIM,iMAX_X_DIM,3),raincum(iMAX_Y_DIM,iMAX_X_DIM)
...
!===============================================================================
! NAMELIST INPUT
NAMELIST /INPUT/ sCTL_TEMP, sCTL_RELH, sCTL_WIND, sCTL_PIOG,           &
                 sCTL_ORO, sCTL_FWI,                                   &
                 sPRECILOG, sERRORLOG, sPREFWILOG,                     &
                 sFWIMETEO, sPROGLOG,                                  &
                 sTEMP_ANA_ABRV, sRELH_ANA_ABRV, sVELU_ANA_ABRV,       &
                 sVELV_ANA_ABRV, sPIOG_ANA_ABRV,                       &
                 sTEMP_IDI_ABRV, sRELH_IDI_ABRV, sVELU_IDI_ABRV,       &
                 sVELV_IDI_ABRV, sPIOG_IDId_ABRV, sPIOG_IDIw_ABRV,     &
                 sORO_TOPO_ABRV
!-------------------------------------------------------------------------------
!===============================================================================
! [0] Set up
...
! [0.1] pointers initialization
  ...
  ALLOCATE( svABRV_FWI(iNVARS_FWI), svDSCR_FWI(iNVARS_FWI), &
              STAT=iRetCode )
  IF (iRetCode/=0) THEN
    PRINT *,'fwigrid: Error in dynamic allocation memory operation for arrays &
      "svABRV_FWI"/"svDSCR_FWI"/"ivVLEVS_FWI" !'
    STOP 1
  ENDIF
  svABRV_FWI(1)  = 'i1'
...
! [1.1] Read Initialization File
  ...
  IF (iRetCode/=0) THEN
    PRINT *,'fwigrid: error opening file <',TRIM(sFILEINI),'>'
    STOP 1
  ENDIF
  READ(10,NML=INPUT)
  CLOSE(10)
  sCTL_ORO=TRIM(ADJUSTL(sCTL_ORO))
...
! [1.2] Check input arguments consistency
 ...
 CALL NameCTL2DAT( sCTL_FWI, sDAT_FWI, lOKflag)
  IF (.not.lOKflag) THEN
    ...
    STOP 1
  ENDIF
  OPEN(31,FILE=TRIM(sDAT_FWI),FORM='BINARY',ACTION='WRITE',IOSTAT=iRetCode)
  IF (iRetCode/=0) THEN
...
! open DAT input GRIDDED geographical file / read geographical info
  OPEN(100,FILE=TRIM(sDAT_ORO),FORM='BINARY',ACTION='READ',IOSTAT=iRetCode)
  IF (iRetCode/=0) THEN
...
!===============================================================================
! [8.1] open input DAT file
  OPEN(101,FILE=TRIM(sDAT_TEMP),FORM='BINARY',ACTION='READ',IOSTAT=iRetCode)
  IF (iRetCode/=0) THEN
    PRINT *,'fwigrid: Error in opening file ',TRIM(sDAT_TEMP)
    STOP 1
  ENDIF
  OPEN(111,FILE=TRIM(sDAT_RELH),FORM='BINARY',ACTION='READ',IOSTAT=iRetCode)
  IF (iRetCode/=0) THEN
    PRINT *,'fwigrid: Error in opening file ',TRIM(sDAT_RELH)
    STOP 1
  ENDIF
  OPEN(121,FILE=TRIM(sDAT_WIND),FORM='BINARY',ACTION='READ',IOSTAT=iRetCode)
  IF (iRetCode/=0) THEN
    PRINT *,'fwigrid: Error in opening file ',TRIM(sDAT_WIND)
    STOP 1
  ENDIF
  OPEN(131,FILE=TRIM(sDAT_PIOG),FORM='BINARY',ACTION='READ',IOSTAT=iRetCode)
  IF (iRetCode/=0) THEN
...
!  sPATH1=TRIM(ADJUSTL(sPATH1))  modificato: ora letti da file fwigrid_ana.ini
!  sPATH2=TRIM(ADJUSTL(sPATH2))  modificato: ora letti da file fwigrid_ana.ini

! lettura file fwigrid_ana.ini

  OPEN(11,file=TRIM(sPATH0)//"ini/fwigrid_ana.ini",status="old",IOSTAT=iRetCode)
  IF (iRetCode/=0) THEN
...
!  lettura file punti di griglia appartenenti alla regione Lombardia (shapefile usato da GRASS)
  ...
  OPEN(15,file=TRIM(sPATH0)//"ini/lombardia.txt",status="old",IOSTAT=iRetCode)
  IF (iRetCode/=0) THEN
...
! lettura file con elenco punti della Lombardia "Grass" non appartenenti al grigliato OI meteo (per differenze di risoluzione nel file originale)
  ...
  OPEN(23,file=TRIM(sPATH0)//"ini/punti_nometeo.ini",status="old",IOSTAT=iRetCode)
  IF (iRetCode/=0) THEN
...
! Individuazione primo e ultimo giorno per calcolo FWI (dopo 24 ore dall'inizio del ciclo orario sulla variabile it)
    ...
    IF ( ASSOCIATED(ivVLEVS_TMP) ) DEALLOCATE(ivVLEVS_TMP)
    NULLIFY(ivVLEVS_TMP)
    ALLOCATE(ivVLEVS_TMP(iNVARS_TEMP),STAT=iRetCode)
    IF (iRetCode/=0) THEN
      PRINT *,'fwigrid: Error allocating ivVLEVS_TMP'
      STOP 1
    ENDIF
    ivVLEVS_TMP=0
    CALL ReadDAT_GRD( 101, rvDATA_SFC_TMP, rvDATA_TMP,                 &
                      iNX, iNY, iNZ,                                   &
                      iNVARS_TEMP, iNVARS_LD_TEMP, iNVARS_SFC_TEMP,    &
                      ivVLEVS_TMP, rUNDEF_TEMP, lOKflag )
    IF (.not.lOKflag) THEN
      PRINT *,'fwigrid: Error in ReadDAT_GRD, file DAT = ',TRIM(sDAT_TEMP)
      STOP 1
    ENDIF
    DO ii=1,iNX
      DO jj=1,iNY
        rvTEMP_ANA_GRID(ii,jj)=rvDATA_SFC_TMP(ii,jj,iTEMP_ANA_POS)
        rvTEMP_IDI_GRID(ii,jj)=rvDATA_SFC_TMP(ii,jj,iTEMP_IDI_POS)
      ENDDO
    ENDDO
    ! [8.2.2] Read relative humidity data
    IF ( ASSOCIATED(ivVLEVS_TMP) ) DEALLOCATE(ivVLEVS_TMP)
    ...
    IF ( ASSOCIATED(ivVLEVS_TMP) ) DEALLOCATE(ivVLEVS_TMP)
    NULLIFY(ivVLEVS_TMP)
    ALLOCATE(ivVLEVS_TMP(iNVARS_WIND),STAT=iRetCode)
    IF (iRetCode/=0) THEN
      PRINT *,'fwigrid: Error allocating ivVLEVS_TMP'
      STOP 1
    ENDIF
    ivVLEVS_TMP=0
    CALL ReadDAT_GRD( 121, rvDATA_SFC_TMP, rvDATA_TMP,                 &
                      iNX, iNY, iNZ,                                   &
                      iNVARS_WIND, iNVARS_LD_WIND, iNVARS_SFC_WIND,    &
                      ivVLEVS_TMP, rUNDEF_WIND, lOKflag )
    IF (.not.lOKflag) THEN
      PRINT *,'fwigrid: Error in ReadDAT_GRD, file DAT = ',TRIM(sDAT_WIND)
      STOP 1
    ENDIF
    DO ii=1,iNX
      DO jj=1,iNY
        rvVELV_ANA_GRID(ii,jj)=rvDATA_SFC_TMP(ii,jj,iVELV_ANA_POS)
        rvVELV_IDI_GRID(ii,jj)=rvDATA_SFC_TMP(ii,jj,iVELV_IDI_POS)
        rvVELU_ANA_GRID(ii,jj)=rvDATA_SFC_TMP(ii,jj,iVELU_ANA_POS)
        rvVELU_IDI_GRID(ii,jj)=rvDATA_SFC_TMP(ii,jj,iVELU_IDI_POS)
      ENDDO
    ENDDO
    ! [8.2.4] Read rain data
    IF ( ASSOCIATED(ivVLEVS_TMP) ) DEALLOCATE(ivVLEVS_TMP)
    NULLIFY(ivVLEVS_TMP)
    ALLOCATE(ivVLEVS_TMP(iNVARS_PIOG),STAT=iRetCode)
    IF (iRetCode/=0) THEN
      PRINT *,'fwigrid: Error allocating ivVLEVS_TMP'
      STOP 1
    ENDIF
    ivVLEVS_TMP=0
    CALL ReadDAT_GRD( 131, rvDATA_SFC_TMP, rvDATA_TMP,                 &
                      iNX, iNY, iNZ,                                   &
                      iNVARS_PIOG, iNVARS_LD_PIOG, iNVARS_SFC_PIOG,    &
                      ivVLEVS_TMP, rUNDEF_PIOG, lOKflag )
    IF (.not.lOKflag) THEN
      PRINT *,'fwigrid: Error in ReadDAT_GRD, file DAT = ',TRIM(sDAT_PIOG)
      STOP 1
    ENDIF
    DO ii=1,iNX
      DO jj=1,iNY
        rvPIOG_ANA_GRID(ii,jj)=rvDATA_SFC_TMP(ii,jj,iPIOG_ANA_POS)
        rvPIOG_IDId_GRID(ii,jj)=rvDATA_SFC_TMP(ii,jj,iPIOG_IDId_POS)
        rvPIOG_IDIw_GRID(ii,jj)=rvDATA_SFC_TMP(ii,jj,iPIOG_IDIw_POS)
        IF ( (rvPIOG_ANA_GRID(ii,jj)>=0.).AND. &
             (rvPIOG_IDId_GRID(ii,jj)>=0.).AND. &
             (rvPIOG_IDIw_GRID(ii,jj)>=0.) ) THEN
          rvPIOG24_ANA_GRID(ii,jj)=rvPIOG24_ANA_GRID(ii,jj)+rvPIOG_ANA_GRID(ii,jj)
          rvPIOG24_IDId_GRID(ii,jj)=rvPIOG24_IDId_GRID(ii,jj)+rvPIOG_IDId_GRID(ii,jj)
          rvPIOG24_IDIw_GRID(ii,jj)=rvPIOG24_IDIw_GRID(ii,jj)+rvPIOG_IDIw_GRID(ii,jj)
        ENDIF
      ENDDO
    ENDDO
    IF (iHH1/=12) CYCLE
...
!  Trattamento IDI (N.B. u e v hanno stesso idi!!!)
        ...
        IF(rvTEMP_IDI_GRID(ic,(iRIGok+1-ir)).eq.rUNDEF_TEMP) rvTEMP_IDI_GRID(ic,(iRIGok+1-ir))=rUNDEF          ! N.B. l'idi nei file grads ha lo stesso UNDEF del parametro cui si riferisce
        IF(rvRELH_IDI_GRID(ic,(iRIGok+1-ir)).eq.rUNDEF_RELH) rvRELH_IDI_GRID(ic,(iRIGok+1-ir))=rUNDEF          ! Percio' qui effettuo la conversione dei codici di valore indefinito in rUNDEF unico
        IF(rvVELU_IDI_GRID(ic,(iRIGok+1-ir)).eq.rUNDEF_WIND) rvVELU_IDI_GRID(ic,(iRIGok+1-ir))=rUNDEF
        IF(rvPIOG24_IDIw_GRID(ic,(iRIGok+1-ir)).eq.rUNDEF_PIOG.or.rvPIOG24_IDId_GRID(ic,(iRIGok+1-ir)).eq.rUNDEF_PIOG) THEN
          idi(4)=rUNDEF
        ELSE
          idi(4)=(rvPIOG24_IDIw_GRID(ic,(iRIGok+1-ir))+rvPIOG24_IDId_GRID(ic,(iRIGok+1-ir)))/24
        END IF

        idi(1)=rvTEMP_IDI_GRID(ic,(iRIGok+1-ir))
        idi(2)=rvRELH_IDI_GRID(ic,(iRIGok+1-ir))
        idi(3)=rvVELU_IDI_GRID(ic,(iRIGok+1-ir))
...
! assegnazione dati meteo
        ...
        t=rvTEMP_ANA_GRID(ic,(iRIGok+1-ir))
        h=rvRELH_ANA_GRID(ic,(iRIGok+1-ir))
        IF(rvVELU_ANA_GRID(ic,(iRIGok+1-ir)).eq.rUNDEF_WIND.or.rvVELV_ANA_GRID(ic,(iRIGok+1-ir)).eq.rUNDEF_WIND) THEN
          w=rUNDEF_WIND
          ws(ir,ic)=rUNDEF_WIND
        ELSE
          w=(sqrt((rvVELU_ANA_GRID(ic,(iRIGok+1-ir)))**2+(rvVELV_ANA_GRID(ic,(iRIGok+1-ir)))**2))*3.6
          ws(ir,ic)=(sqrt((rvVELU_ANA_GRID(ic,(iRIGok+1-ir)))**2+(rvVELV_ANA_GRID(ic,(iRIGok+1-ir)))**2))*3.6
        END IF
        r=rvPIOG24_ANA_GRID(ic,(iRIGok+1-ir))
        tok=t
...
! **********
            ...
            ELSE IF((iSNOWold(ir,ic).eq.0.and.ind_tmp(ir,ic,1).eq.rUNDEF_SNOW).or.&
                     (iSNOWold(ir,ic).eq.0.and.ind_tmp(ir,ic,1).ne.rUNDEF_SNOW.and.(isciolgo(ir,ic).gt.0.and.isciolgo(ir,ic).le.istdby))) THEN
 ! incertezza ieri, con indice_temp di ieri uguale rUNDEFSNOW oppure con isciolgo <=istdby -> niente neve ieri -> nessuna variazione="da non-neve a non-neve"
          ...
          ELSE IF(iSNOWtod(ir,ic).eq.0) THEN  ! se il punto e' incerto oggi
            IF(iSNOWold(ir,ic).eq.2.or.(iSNOWold(ir,ic).eq.0.and.(ind_tmp(ir,ic,1).ne.rUNDEF_SNOW.or.isciolgo(ir,ic).le.istdby))) THEN ! in caso di innevamento ieri\o prima -> innevamento oggi
              mc=rSNOWCODE
              ffm=rSNOWCODE
              dc=rSNOWCODE
              dmc=rSNOWCODE
...
!                                       la sezione di "overwintering"
    ...
    DO kk=1,6
      ...
      DO ir=1,iRIGok
        WRITE(iFILO(kk),"(<iCOLok>(1x,f11.5))") (rvFWIindexes(ic,(iRIGok+1-ir),kk), ic=1,iCOLok)
      END DO
      CLOSE(iFILO(kk))
    END DO
...
!                             N.B. tali valori sono calcolati classificando, mediante il file fwiclassi.ini, i valori ottenuti al punto precedente (ffmc_grezzi).
    ...
    DO kk=1,6
      ...
      DO ir=1,iRIGok
        WRITE(iFILOC(kk),"(<iCOLok>(1x,i5))") (iFWIind_class(ic,(iRIGok+1-ir),kk), ic=1,iCOLok)
      END DO
      CLOSE(iFILOC(kk))
    END DO
...
!  file idi complessivo del giorno corrente
    ...
    DO ir=1,iRIGok
      WRITE(99,"(<iCOLok>(1x,i5))") (iditot(ir,ic), ic=1,iCOLok)
    END DO
    CLOSE(99)
...
! file indici temporanei e precipitazioni totali cumulate temporanee: per trattamento "neve al suolo".
    ...
    DO kk=1,3
      ...
      DO ir=1,iRIGok
        WRITE(iFILOT(kk),"(<iCOLok>(1x,f11.5))") (ind_tmp(ir,ic,kk), ic=1,iCOLok)
      END DO
      CLOSE(iFILOT(kk))
    END DO
    ...
    DO ir=1,iRIGok
      WRITE(94,"(<iCOLok>(1x,f7.1))") (raincum(ir,ic), ic=1,iCOLok)
    END DO
    CLOSE(94)
...
!  temperatura
    ...
    DO ir=1,iRIGok
      WRITE(122,"(<iCOLok>(1x,f7.1))") (rvTEMP_ANA_GRID(ic,(iRIGok+1-ir)), ic=1,iCOLok)
    END DO
    CLOSE(122)
...
!  umidita' relativa
    ...
    DO ir=1,iRIGok
      WRITE(123,"(<iCOLok>(1x,f7.1))") (rvRELH_ANA_GRID(ic,(iRIGok+1-ir)), ic=1,iCOLok)
    END DO
    CLOSE(123)
...
!  velocita' vento
    ...
    DO ir=1,iRIGok
      WRITE(124,"(<iCOLok>(1x,f7.1))") (ws(ir,ic), ic=1,iCOLok)
    END DO
    CLOSE(124)
...
!  Precipitazioni totali
    ...
    DO ir=1,iRIGok
      WRITE(125,"(<iCOLok>(1x,f7.1))") (rvPIOG24_ANA_GRID(ic,(iRIGok+1-ir)), ic=1,iCOLok)
    END DO
    CLOSE(125)
...
!  file "periodo innevamento" del giorno corrente
    ...
    DO ir=1,iRIGok
      WRITE(78,"(<iCOLok>(1x,i5))") (isnowper(ir,ic), ic=1,iCOLok)
    END DO
    CLOSE(78)
...
!  file "codice scioglimento" del giorno corrente
    ...
    DO ir=1,iRIGok
      WRITE(79,"(<iCOLok>(1x,i5))") (isciolgo(ir,ic), ic=1,iCOLok)
    END DO
    CLOSE(79)
