...
# 2010/07/08  CR. aggiornamento che tiene conto del Flag_manuale sul DBmeteo.
...
source(paste(working_directory,'getcsv_recenti_sub.R',sep=""))
...
cat ( "InIZio-=---=-- getcsv_recenti.R =---=-----=---==----iNiZio-----=\n" , file = file_log,append=TRUE)
...
data_oggi<- as.POSIXct( strptime(format(Sys.time(), "%Y%m%d%H%M"),"%Y%m%d%H%M"), "UTC")
data_oggi_meno_15gg<- as.POSIXct( strptime(format(Sys.time(), "%Y%m%d%H%M"),"%Y%m%d%H%M"), "UTC")-3600*24*15
#
if (data_inizio<=data_oggi_meno_15gg){
  cat("ERRORE: E' stata richiamata la routine getcsv_recenti ma la data richiesta non appartiene alle tabelle recenti del DB meteo \n",file=file_log,append=T)
  cat(paste(" data inizio richiesta =",data_inizio," \n"),file=file_log,append=T)
  cat(paste(" data   fine richiesta =",data_fine," \n"),file=file_log,append=T)
  cat(paste(" data             oggi =",data_oggi," \n"),file=file_log,append=T)
  cat(paste(" data     oggi - 15 gg =",data_oggi_meno_15gg," \n"),file=file_log,append=T)
  cat("\"data inizio richiesta\" deve essere maggiore di \"data oggi - 15 gg\" \n",file=file_log,append=T)
  quit(status=1)
}
