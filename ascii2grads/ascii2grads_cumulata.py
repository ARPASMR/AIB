...
def legge_e_somma(file_i,cumulata):
    ...
    while k<len(righe):
        try:
            ...
            while j<len(righe[k]):
                ...
                if float(righe[k][j])>=0:
                    cumulata[k][j]=cumulata[k][j]+float(righe[k][j])
                j+=1
            k+=1
        except:
            j=0
            ...
            while j<len(righe[k]):
                #print "*****************************"
                #print "Qua ci entro solo al tempo 1"
                #print "*****************************"
                #print "k:{0},j:{1}".format(k,j)
                #print righe[k][j]
                riga_temp.append(float(righe[k][j]))
                if j==(len(righe[k])-1):
                    cumulata.append(riga_temp)
                j+=1
            k+=1
            #print k
            #print '*****************************'
...
def legge_e_stampa(file_i,file_o,cumulata):
    f_i = open(file_i, 'r') # 'r' = read
    #print "Ok 0"
    #print file_i
    righe=[]
    ...
    #print len(righe)
    k=0
    while k<len(righe): #ciclo su righe
        j=0
        while j<len(righe[k]):
            if float(righe[k][j])>=0:
                cumulata[k][j]=cumulata[k][j]+float(righe[k][j])
    ...
    while k<=len(righe):
        ...
        while j<len(righe[riga_w]):
            ...
            b=struct.pack('f',cumulata[riga_w][j])
            ff.write(b)
            ff.flush()
...
def main():
    ...
    #data_file_milanone=data_read.strftime("%Y%m%d")
    #nomefile_milanone="{0}/{1}{2}.dat".format(nome_path_milanone,data_file_milanone,title_output)
    ####################################################################################
    ...
    # in questo caso scrivo solo l'ora finale (precipitazione cumulata)
    ctl.write("\nTDEF 1 LINEAR {1} {0}HR".format(interval,data_write))
    ctl.write("\nVARS 1")
    ...
    while t <= int(time_steps):
        ...
        if t<int(time_steps):
            #print cumulata
            legge_e_somma(nomefile, cumulata)
        else:
            legge_e_stampa(nomefile, nomefile_dat, cumulata)
        #exit()
        t=t+int(interval)
